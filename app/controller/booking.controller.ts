    ...
    //   ];
      ...
      function availableTimeSlotsForBooking(durationInMinutes: number) {
        ...
        for (const photographer of data.photographers) {
          const timeSlot = photographer.availabilities.find(
            (availability: { starts: string; ends: string }) => {
              ...
              const bookingsInSlot = photographer.bookings.filter(
                (booking: { id: string; starts: string; ends: string }) => {
                  return (
        ...
        return { photographers: availablePhotographers };
      }
      
      console.log(JSON.stringify(availableTimeSlotsForBooking(190)));
}
